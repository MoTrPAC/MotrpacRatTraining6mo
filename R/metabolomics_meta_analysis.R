...
#'   \item{\code{assay}}{character, assay code ("metab")}
#'   \item{\code{sex}}{`r sex()`}
...
#'   \item{\code{effect_size}}{`r logFC()`}
#'   \item{\code{effect_size_se}}{`r logFC_se()`}
...
#'     Higher values mean the measurements between sites were more discordant.}
#'   \item{\code{QEp}}{double, heterogeneity p-value. A smaller p-value means
#'     the measurements between sites were more discordant.} 
...
#' @export
#' 
#' @seealso [load_metabolomics_da()], [MotrpacRatTraining6moData::METAB_DA]
#' @details
#' To see the expected format of a custom input, look at a table returned by [load_metabolomics_da()]. 
#'
#' @examples
#' # Perform meta-analysis on gastrocnemius timewise summary statistics
#' res = metabolomics_meta_analysis_timewise("SKM-GN")
metabolomics_meta_analysis_timewise = function(tissue, input=NULL){
  ...
  setnames(merged_results, c("effect_size","effect_size_se","zscore"), c("logFC","logFC_se","tscore"))
...
get_meta_analysis_results = function(name, x_subset, subset_names){
  ...
  res = metafor::rma.uni(yi = x_subset$effect_size,
                sei = x_subset$effect_size_se,method = "FE",
                slab = labs)
