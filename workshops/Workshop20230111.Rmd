...
<!-- However, the nominal p-values should be nearly identical unless the pathway databases changed, -->
<!-- so let's do a sanity check. -->
<!-- ```{r compare} -->
<!-- old_res = GRAPH_PW_ENRICH[GRAPH_PW_ENRICH$cluster %in% names(MY_CLUSTERS),] -->
<!-- new_res = res -->
<!-- merged = merge(old_res,  -->
<!--                new_res,  -->
<!--                by=c("term_id","source","term_name","cluster","tissue","ome"),  -->
<!--                suffixes=c("_old","_new")) -->
<!-- ggplot(merged, aes(x=-log10(computed_p_value_old), y=-log10(computed_p_value_new))) + -->
<!--   geom_point(aes(colour=source)) + -->
<!--   geom_abline(linetype="dashed") + -->
<!--   theme_bw() -->
<!-- ggplot(merged, aes(x=-log10(adj_p_value_old), y=-log10(adj_p_value_new))) + -->
<!--   geom_point(aes(colour=source)) + -->
<!--   geom_abline(linetype="dashed") + -->
<!--   theme_bw() -->

<!-- # are input sizes the same at least? -->
<!-- ggplot(merged, aes(x=query_size_old, y=query_size_new)) + -->
<!--   geom_point(aes(colour=source)) + -->
<!--   geom_abline(linetype="dashed") + -->
<!--   theme_bw() -->
<!-- # background sizes? -->
<!-- ggplot(merged, aes(x=effective_domain_size_old, y=effective_domain_size_new)) + -->
<!--   geom_point(aes(colour=source)) + -->
<!--   geom_abline(linetype="dashed") + -->
<!--   theme_bw() -->
<!-- # pathway sizes? -->
<!-- ggplot(merged, aes(x=term_size_old, y=term_size_new)) + -->
<!--   geom_point(aes(colour=source)) + -->
<!--   geom_abline(linetype="dashed") + -->
<!--   theme_bw() -->

<!-- # biggest difference is in effective domain size, but gene universes are identical -->
<!-- # best guess: -->
<!-- ``` -->
...
<!-- ### Protein expression data in a single tissue -->
<!-- ```{r pca prot single tissue} -->
<!-- # load data -->
<!-- prot_kidney = load_sample_data("KIDNEY","PROT",exclude_outliers=TRUE) -->
<!-- head(prot_kidney) -->
<!-- # convert to numeric matrix -->
<!-- rownames(prot_kidney) = prot_kidney$feature_ID -->
<!-- prot_kidney[,c("feature","feature_ID","tissue","assay")] = NULL -->

<!-- # perform PCA -->
<!-- pca = prcomp(t(prot_kidney), center=TRUE) -->

<!-- # handle missing values  -->
<!-- nrow(prot_kidney) -->
<!-- table(complete.cases(prot_kidney)) -->
<!-- # remove genes with any missing values. -->
<!-- # this is crude filtering for the sake of this tutorial. -->
<!-- # in practice, we would impute missing values  -->
<!-- prot_kidney = prot_kidney[complete.cases(prot_kidney),] -->
...
<!-- # merge PCs with PHENO -->
<!-- pcs = as.data.frame(pca$x) -->
<!-- pheno = unique(PHENO[,c("viallabel","group","sex")]) -->
<!-- head(pheno) -->
<!-- data_for_plotting = merge(pheno, pcs, by.x="viallabel", by.y="row.names") -->

<!-- # minimal -->
<!-- ggplot(data_for_plotting, aes(x=PC1, PC2, shape=sex)) + -->
<!--   geom_point() -->
...
<!--   theme_bw() + -->
<!--   guides(fill=guide_legend(override.aes = list(shape=21))) + -->
<!--   labs(title="PCA of kidney protein expression data") + -->
<!--   theme(plot.title = element_text(hjust=0.5)) -->
<!-- ``` -->

<!-- ### All gene expression data -->
<!-- ```{r pca gene expression} -->
<!-- # merge normalized sample-level data from all tissues -->
<!-- merged_data = NULL -->
<!-- for(tissue in TISSUE_ABBREV){ -->
<!--   norm = load_sample_data(tissue, assay="TRNSCRPT", exclude_outliers = TRUE) -->
<!--   if(is.null(norm)){ -->
<!--     next -->
<!--   } -->
<!--   # convert norm to numeric matrix -->
...
<!--     next -->
<!--   } -->
<!--   # note that any genes not expressed in all tissues are excluded -->
<!--   merged_data = merge(merged_data, norm, by="row.names") -->
...
<!-- # perform PCA  -->
<!-- pca = prcomp(t(merged_data), center = TRUE) -->
...
<!--   scale_shape_manual(values=c(male=21, female=24)) + -->
<!--   theme_bw() + -->
<!--   guides(fill=guide_legend(override.aes = list(shape=21, size=3), ncol=2)) + -->
<!--   labs(title="PCA of RNA-seq data") + -->
<!--   theme(plot.title = element_text(hjust=0.5)) -->
<!-- ``` -->

<!-- Given all of the samples and colors, it's hard to distinguish samples from each tissue.  -->
<!-- Let's print the same PC plot, highlighting one tissue at a time.  -->
<!-- ```{r, out.width="33%", out.height="33%"} -->
<!-- # highlight one tissue at a time? -->
<!-- for(tissue in unique(data_for_plotting$tissue)){ -->
<!--   # add new colour column -->
<!--   data_for_plotting$colour = factor(ifelse(data_for_plotting$tissue == tissue, TISSUE_COLORS[[tissue]], "gray"), -->
<!--                                     levels = c("gray", TISSUE_COLORS[[tissue]])) -->
<!--   # reorder points so this tissue is on top  -->
<!--   data_for_plotting = data_for_plotting[order(data_for_plotting$colour),] -->
<!--   g = ggplot(data_for_plotting, aes(x=PC1, y=PC2, fill=colour, shape=sex)) +  -->
<!--     geom_point() + -->
<!--     scale_fill_identity() + -->
<!--     scale_shape_manual(values=c(male=21, female=24)) + -->
<!--     theme_bw() + -->
<!--     labs(title=sprintf("PCA of RNA-seq data (%s)", tissue)) + -->
<!--     theme(plot.title = element_text(hjust=0.5)) -->
<!--   print(g) -->
<!-- } -->
<!-- ``` -->

<!-- ## Compare protein expression and gene expression for the same samples -->
<!-- ```{r prot vs rna} -->
<!-- # for each ome, download data, melt, add pheno, add gene symbol -->
<!-- data_list = list() -->
<!-- data("FEATURE_TO_GENE_FILT") -->
<!-- for(ome in c("PROT", "TRNSCRPT")){ -->
<!--   data = load_sample_data("HEART", ome) -->

<!--   # add gene symbol -->
<!--   data2 = merge(data, FEATURE_TO_GENE_FILT[,c("feature_ID", "gene_symbol")], by="feature_ID") -->
<!--   # for each gene symbol, keep the highest expression feature -->
<!--   data2$mean_expr = apply(data2[,colnames(data2)[grepl("^9", colnames(data2))]], 1, mean, na.rm=TRUE) -->
<!--   data2 = data2[order(data2$mean_expr, decreasing=TRUE),] -->
<!--   data2 = data2[!duplicated(data2$gene_symbol),] -->

<!--   # melt -->
<!--   melted = reshape2::melt(data2,  -->
<!--                           id.vars = c("feature_ID","tissue","assay","gene_symbol"), -->
<!--                           measure.vars = colnames(data)[grepl("^9", colnames(data))], -->
<!--                           variable.name = "viallabel", -->
<!--                           value.name = ome) -->

<!--   # add pheno -->
<!--   pheno = unique(PHENO[,c("pid","viallabel","group","sex")]) -->
<!--   merged = merge(pheno, melted, by="viallabel") -->

<!--   # remove missing values  -->
<!--   merged = merged[complete.cases(merged),] -->

<!--   data_list[[ome]] = merged -->
<!-- } -->

<!-- # merge RNA and PROT data -->
<!-- merged = merge(data_list[["PROT"]], data_list[["TRNSCRPT"]], -->
<!--                by=c("pid","group","sex","tissue","gene_symbol"), -->
<!--                suffixes=c("_prot","_rna")) -->
<!-- nrow(merged) -->

<!-- # subset to just control females -->
<!-- merged = merged[merged$group == "control" & merged$sex == "female",] -->

<!-- # plot PROT vs TRNSCRPT values for control females -->
<!-- ggplot(merged, aes(x=PROT, y=TRNSCRPT)) + -->
<!--   geom_point(size=0.1, alpha=0.5) + -->
<!--   theme_bw() + -->
<!--   labs(title="Heart protein vs. transcript expression (control females)")  -->

<!-- # plot PROT vs TRNSCRPT values for a single animal -->
<!-- ggplot(merged[merged$pid == "10043527",], aes(x=PROT, y=TRNSCRPT)) + -->
<!--   geom_point(size=0.1, alpha=0.5) + -->
<!--   theme_bw() + -->
<!--   labs(title="Heart protein vs. transcript expression (control female 10043527)")  -->
<!-- ``` -->
