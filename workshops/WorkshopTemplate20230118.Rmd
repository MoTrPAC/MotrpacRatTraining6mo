...
# make a named list where names are tissue:path and values are features in that cluster
clusters = list()
paths = names(sets)
...
names(MY_CLUSTERS)
# make a network for the "all up" gastroc cluster
?enrichment_network_vis

```

```{r viznetwork heart, fig.width=8}
# make a network for the "all up" heart cluster
...
# make a network for "all up" cluster in both heart and gastroc
