...
preprint will be added [here](https://motrpac.github.io/MotrpacRatTraining6mo/#citing-motrpac-data) shortly. **We highly recommend skimming the preprint 
...
[MotrpacRatTraining6moData](https://motrpac.github.io/MotrpacRatTraining6moData)
separately before re-running the above command to install this package and its
other dependencies. 
...
For the sake of easily loading and looking up documentation for data objects, 
let's also load the [MotrpacRatTraining6moData](https://motrpac.github.io/MotrpacRatTraining6moData) 
package. 
```{r data lib}
...
`?load_sample_data`. Note that both `MotrpacRatTraining6mo` and 
...
of the [bioRxiv preprint](https://motrpac.github.io/MotrpacRatTraining6mo/#citing-motrpac-data). Briefly, 6-month-old young adult rats 
...
is loaded, you can learn more about any of these data objects with `?`, e.g., 
...
supplementary methods of the [bioRxiv preprint](https://motrpac.github.io/MotrpacRatTraining6mo/#citing-motrpac-data). 
...
see the supplementary methods in the [bioRxiv preprint](https://motrpac.github.io/MotrpacRatTraining6mo/#citing-motrpac-data). 
...
enrichment was performed for the [preprint](https://motrpac.github.io/MotrpacRatTraining6mo/#citing-motrpac-data), namely the 2 largest nodes, 
...
by protein phosphosites. We also see a cluster comprised mostly of protein 
...
(`1w_F-1_M0->2w_F-1_M0->4w_F-1_M0->8w_F-1_M-1`). In the next section, we show how 
...
[preprint](https://motrpac.github.io/MotrpacRatTraining6mo/#citing-motrpac-data) are provided in `GRAPH_PW_ENRICH`. 
...
nrow(res2[res2$adj_p_value < 0.05,])
# split by tissue and ome
significant = res2[res2$adj_p_value < 0.05,]
table(significant$tissue, significant$ome)
```

Note that the number of significantly enriched pathways for some non-METAB omes is 
less than in the previous example. This is a necessary consequence of performing 
multiple testing adjustment with more tests. 
...
```{r viznetwork1, fig.width=10}
# Extract pathway enrichments for this cluster
